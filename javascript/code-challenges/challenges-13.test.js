...
const longestString = (arr) => {
  var longest;
  arr.reduce((bank, current) => bank.length > current.length ? longest = bank : longest = current,'');
...
const findHappiness = (arr) => {
  const happyArr =[];
  arr.forEach( str => str.includes(":)") ? happyArr.push(str) : happyArr);
  return happyArr;};
...
const onlyOddChars = (str) => {
  let oddArr =[];
  str.split('').reduce( (bank, current, i) => i % 2 === 1 ? oddArr.push(current) : oddArr, []);
  return oddArr.join('');
};
