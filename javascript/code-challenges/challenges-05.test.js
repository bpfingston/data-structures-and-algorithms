...
const addValues = (arr) => arr.reduce((prev, current) => {return prev + current;}, 0);
...
const addPurchases = (arr) => arr.reduce((valA, valB) => {return valA + valB.purchasePrice;}, 0);
...
const countNumberOfElements = (arr) => arr.reduce(ele => {return ++ele;}, 0);
...
const returnNames = (arr) => arr.reduce( (position, obj) => position.concat(obj.name), []);
...
const countNumberOfChildren = (arr) => arr.reduce((kids, person) => { return person.children ? kids + person.children.length: kids;},0);
...
const countPrimeNumbers = (arr) => arr.reduce((prev, current) => {return isPrime(current) ? ++prev : prev;}, 0);
...
const extractStat = (statName, arr) => arr.reduce((hidden, original) => {return original.stat.name === statName ? hidden = original : hidden;}, null);
...
const extractChildren = (arr) => {
  return arr.filter( got => {
    return got.name.includes('a');})
    .reduce((kids, parent) => {
      return parent.children ? kids.concat(parent.children) : kids;
    },[]);
};
