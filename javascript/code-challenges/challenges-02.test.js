...
const charCode = (arr) => {
  return arr.map(code => code.charCodeAt(0));
};
...
const extractAbilities = (arr) => arr.map((pokemon) => pokemon.ability.name);
...
const extractStats = (arr) => arr.map((pokemon) => (
  {
    name: pokemon.stat.name, total: pokemon.effort + pokemon.baseStat
  }
));
