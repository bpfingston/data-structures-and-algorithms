...
const count = (target, input) => {
  var int = 0;
  input.flat().filter(num => num === target ? int++ : int);
  return int;
};
...
const totalSum = (input) => input.flat().reduce((bank, current) => bank + current,0);
...
let findMaleAndFemale = (data) => {
  ...
  data.forEach((ele, i, arr) => {
    if(ele.gender === 'male' || (ele.gender ==='female')){
      if(i === arr.length - 1){
        starArr.push(`${ele.name}`);
      } else {
        starArr.push(`${ele.name} and `);
      }
    }
  });
  return starArr.join('');
